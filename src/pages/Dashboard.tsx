...
import { Shop, formatReportQty, sortLensNames, getDefaultShopId } from '../utils/lensUtils';
  ...
  useEffect(() => {
    async function fetchShops() {
      ...
      if (shopsData && shopsData.length > 0) {
        ...
        // ✅ Default Shop Mapping — email ke hisaab se default shop select karo
        const { data: { user } } = await supabase.auth.getUser();
        const email = user?.email || '';
        setSelectedShop(getDefaultShopId(shopsData, email));
      }
      setLoading(false);
    }
    fetchShops();
